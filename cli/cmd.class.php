...
*/
	...
	*/
	...
	class CMD
	{
		 ...
		 * @param string|int $style style (number or name) (0=reset, 1=bold, 4=underline, 7=reverse)
		...
		public function center(string $text, string $char = ' ', int $spaceLen = 1, int $marginLen = 0, string|int $color = 39, string|int $style = 0, bool $light = true ) :string
		{
			...
			$styledText = $this->style( $text, $color, $style, $light );

			return PHP_EOL . str_pad(
				str_pad(
					str_pad(
						$styledText,
						strlen($styledText) + 2*$spaceLen,
						' ',
						STR_PAD_BOTH
				...
				' ',
				STR_PAD_BOTH
			); // pad box with spaces (to seperate it from cmd border)
		}
		 ...
		 * @param array|string $char string that will: fill whole 1st. and last line, fill partly 2nd and 4th line, fill empty space of middle line.\nIf $char is array, [0] will fill 1st line, [1] second, [2] third etc.
			...
			$recursive = function( $list, $depth ) use ( $bullet, $last_bullet, &$recursive )
			{
				foreach ( $list as $element )
				{
					...
					else
					{
						...
						{
							if( isset($bullet[$depth][1]) && ($bullet[$depth][1] & 1) )	//ITERABLE
